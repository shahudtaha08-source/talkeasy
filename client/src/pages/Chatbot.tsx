...
import { useConversations, useConversationHistory, useCreateConversation } from "@/hooks/use-chat";
...
export default function Chatbot() {
  ...
  const { data: history, isLoading: loadingHistory } = useConversationHistory(activeId);
  ...
  const handleNewChat = async () => {
    const newConvo = await createConvo({ title: `Session ${new Date().toLocaleDateString()}` });
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.content) {
                  ...
                  setMessages(prev => {
                    const newMsgs = [...prev];
                    newMsgs[assistantMessageIdx] = { role: "assistant", content: assistantText };
                    return newMsgs;
                  });
                }
                if (data.done) {
    ...
    <div className="h-[calc(100vh-4rem)] flex gap-6">
      ...
      <div className="flex-1 glass-card rounded-3xl flex flex-col overflow-hidden relative shadow-xl">
        ...
        <div className="p-6 border-b border-border/50 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md">
          ...
          <p className="text-sm text-muted-foreground">Safe, private, and judgment-free space.</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 && !loadingHistory && (
            <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
              ...
              <p className="text-sm">Say hello or share how you're feeling today.</p>
            </div>
          )}
                ...
                <div className={`max-w-[75%] p-4 rounded-2xl ${
                  ...
                  {m.content ? (
                    ...
                    <div className="flex gap-1 items-center h-6">
                      ...
                      <span className="w-2 h-2 rounded-full bg-teal-500 animate-bounce" style={{animationDelay: "0.4s"}}></span>
                    </div>
                  )}
                </div>
              </motion.div>
